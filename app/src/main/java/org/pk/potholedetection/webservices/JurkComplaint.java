...
/**
 * This is a web service class to make complaint.
 * 
 */
public class JurkComplaint extends BaseService {

	private final Complaint _complaint;

	private final String _complaintUrl = CommunicationConstants.SERVER_BASE_URL + "jurk_complain.php";
	 ...
	 */
	public JurkComplaint(Complaint complaint) {
	...
	protected void parseResponse(String response) {
		LogUtils.debug(LogCategory.SERVICE, "JurkComplaint: parseResponse: response->" + response);
		...
		try {
			...
			if (status) {
				...
				Toast.makeText(MainApplication.appContext, description, Toast.LENGTH_LONG).show();
